...
const App = () => {
  const [basePower, setBasePower] = React.useState(0);
  const [atkLevel, setAtkLevel] = React.useState(0);
  const [coins, setCoins] = React.useState([{ power: 0, prob: 1.0, bonus: 0 }]);
  const [commonBonus, setCommonBonus] = React.useState(0);
  const [critBonus, setCritBonus] = React.useState(0);
  ...
  return React.createElement("div", { style: { maxWidth: 600, margin: "0 auto", padding: 20 } }, [
    ...
    ...coins.map((coin, index) =>
      React.createElement("div", { key: index }, [
        React.createElement("input", {
          ...
          placeholder: "코인 위력",
          value: coin.power,
          onChange: e => updateCoin(index, "power", e.target.value),
        }),
        React.createElement("input", {
          type: "number",
          step: "0.01",
          placeholder: "확률",
          value: coin.prob,
          onChange: e => updateCoin(index, "prob", e.target.value),
        }),
        React.createElement("input", {
          type: "number",
          placeholder: "피해량 증가(%)",
          value: coin.bonus,
          onChange: e => updateCoin(index, "bonus", e.target.value),
        }),
      ])
    ),
